...
def compute_centroids_test(target):
    ...
    assert centroids.shape == (
        K, X.shape[1]), f"Wrong shape. Expected: {(len(X),)} got: {idx.shape}"
    assert np.allclose(
        centroids, expected_centroids), f"Wrong values. Expected: {expected_centroids}, got: {centroids}"
...
def find_closest_centroids_test(target):
    ...
    assert idx.shape == (
        len(X),), f"Wrong shape. Expected: {(len(X),)} got: {idx.shape}"
    ...
    assert idx.shape == (
        len(X),), f"Wrong shape. Expected: {(len(X),)} got: {idx.shape}"
    assert np.allclose(
        idx, [1, 1, 2, 2, 0, 0]), f"Wrong values. Expected {[2, 2, 0, 0, 1, 1]}, got: {idx}"
    ...
    assert idx.shape == (
        len(X),), f"Wrong shape. Expected: {(len(X),)} got: {idx.shape}"
    assert np.allclose(idx, [1, 1, 2, 2, 0, 1, 2]
                       ), f"Wrong values. Expected {[2, 2, 0, 0, 1, 1]}, got: {idx}"
